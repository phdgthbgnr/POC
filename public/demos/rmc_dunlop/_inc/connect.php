...
class connect{
  ...
  var $tb2 = 'rmcdunlop_lastid';
    ...
    $this->dbname = "entertaiciggd";
		...
		if(preg_match("/centos7/",$_SERVER['SERVER_NAME'])){
			$this->dbname = "rmcdunlop";
			$this->host = "localhost";
			$this->login = "rmcdunlop";
			$this->pass = "1234";
		}
	...
	function execute_query($s){
		if (!$return = mysql_query($s)){
			echo ('erreur dans la base de données'.mysql_errno()).' ----- '.mysql_error();
			if(mysql_errno() !== 1062){
