...
(function(){
        ...
        function setMaxArrays(){
          _mldf = _formation == 4132 ? 1 : (_formation == 433 ? 1 :2);                           // max array milieux defensifs
          _mloff = _formation == 4132 ? 3 :(_formation == 433 ? 2 :2);  // max array milieux offensifs
        ...
        jQuery(document).ready(function($){
          ...
          */
          ...
          chargeEcran = function(sc,scr){
            var scrol = scr === undefined ? true : scr;
            switch(sc){
              case 1:
              createliste('poste',1,'gardiens',scrol);
              break;
              case 2:
              createliste('poste',2,'defenseursc',scrol);
              break;
              case 3:
              createliste('poste',2,'defenseursl',scrol);
              break;
              case 4:
              createliste('poste',3,'milieuxd',scrol);
              break;
              case 5:
              createliste('poste',3,'milieuxo',scrol);
              break;
              case 6:
              createliste('poste',4,'attaquants',scrol);
              break;
            }
          };
          ...
          $(id).click(function(e){
              ...
              if(_gardiens.length == _mg && _defenseursc.length == _dfc && _defenseursl.length == _dfl && _milieuxd.length == _mldf && _milieuxo.length == _mloff && _attaquants.length == _at){
                ...
                request=$.ajax({
                  ...
                  success: function( res ) {
                    ...
                    elems += '<a href="https://twitter.com/intent/tweet/?text=Tentez%20de%20gagner%20une%20rencontre%20avec%20Raphaël%20Varane%20ou%20une%20enceinte%20JBL%20!%20Voici%20ma%20liste%20des%2011%20!&url=http://votre11bleu.bfmtv.com/liste/'+idjoueur+'/'+tokens+'" class="bouton partage twitter" id="twitter" data-idjoueur="'+idjoueur+'" data-tokens="'+tokens+'">Partager sur Twitter</a>';
            ...
            if(_t.hasClass('selteam')){
              if(!_t.hasClass('selecteam')){
                ...
                elems += '<ul>';
                elems += makeListeteam(dbteam[id].selection['433'], 'gardiens');
                elems += '</ul>';
                elems += '</li>';

                elems += '<li class="poste"><span>Défenseurs</span>';
                elems += '<ul>';
                elems += makeListeteam(dbteam[id].selection['433'], 'defenseursc');
                elems += makeListeteam(dbteam[id].selection['433'], 'defenseursl');
                elems += '</ul>';
                elems += '</li>';

                elems += '<li class="poste"><span>Milieux de terrain</span>';
                elems += '<ul>';
                elems += makeListeteam(dbteam[id].selection['433'], 'milieuxd');
                elems += makeListeteam(dbteam[id].selection['433'], 'milieuxo');
                elems += '</ul>';
                elems += '</li>';

                elems += '<li class="poste"><span>Attaquants</span>';
                elems += '<ul>';
                elems += makeListeteam(dbteam[id].selection['433'], 'attaquants');
                elems += '</ul>';
                elems += '</li>';
              ...
              request=$.ajax({
                ...
                success: function( res ) {
                  ...
                  if(typeof res == 'object'){
                    ...
                    var elems = '';
                    elems += '<div class="formintern">'
                    var intformation = '';
                    var prop = 'formation';
                    if(prop in res) intformation = res.formation;

                    switch (intformation) {
                      case '442':
                        elems += '<strong>4-4-2</strong> à plats avec milieux récupérateurs';
                        break;
                      case '4132':
                        elems += '<strong>4-4-2</strong> en losange';
                        break;
                      case '433':
                        elems += '<strong>4-3-3</strong> pointe basse';
                        break;
                    }

                    elems += '</div>';

                    elems += '<ul class="listeinternautes">';
                    ...
                    var totaldefc = 0;
                    var prop = 'totaldefc';
                    if(prop in res) totaldefc = res.totaldefc;

                    var totaldefl = 0;
                    var prop = 'totaldefl';
                    if(prop in res) totaldefl = res.totaldefl;

                    var totalmilo = 0;
                    var prop = 'totalmilo';
                    if(prop in res) totalmilo = res.totalmilo;

                    var totalmild = 0;
                    var prop = 'totalmild';
                    if(prop in res) totalmild = res.totalmild;
                    ...
                    for(var t in res){
                      switch (t) {
                        case 'gardiens':
                          ...
                          break;
                        case 'defenseursc':
                          elems += '<li>Défenseurs centraux'+makeListeintern(res, 'defenseursc', totaldefc)+'</li>';
                          break;
                        case 'defenseursl':
                            elems += '<li>Défenseurs latéraux'+makeListeintern(res, 'defenseursl', totaldefl)+'</li>';
                            break;
                        case 'milieuxd':
                          elems += '<li>Milieux défensifs'+makeListeintern(res, 'milieuxd', totalmild)+'</li>';
                          break;
                        case 'milieuxo':
                            elems += '<li>Milieux offensifs'+makeListeintern(res, 'milieuxo', totalmilo)+'</li>';
                            break;
                        case 'attaquants':
            ...
            if(_t.hasClass('facebook')){
              FB.ui({
                  method: 'feed',
                  name: 'Voici ma liste des 11 bleus !',
                  link: 'http://votre11bleu.bfmtv.com/liste/'+_idjoueur+'/'+_tokens,
                  picture: 'http://votre11bleu.bfmtv.com/partage-ton-11-2.jpg',
                  description: 'A vous de sélectionner vos 11 bleus pour tenter de gagner une rencontre avec Raphaël Varane à Madrid ou une enceinte JBL édition limitée !',
                  caption: 'Votre 11 bleu'
              },function(d){
                  if(d!=null) {
                    //console.log('d : '+d);
                    if(request) request.abort();
            ...
            if(_t.hasClass('twitter')){
              ...
              url    = 'https://twitter.com/intent/tweet/?text=Tentez%20de%20gagner%20une%20rencontre%20avec%20Raphaël%20Varane%20ou%20une%20enceinte%20JBL%20!%20Voici%20ma%20liste%20des%2011%20!&url=http://votre11bleu.bfmtv.com/liste/'+_idjoueur+'/'+_tokens,
              opts   = 'status=1' +
              ',width='  + width  +
              ',height=' + height +
              ',top='    + top    +
