var playState = {
  ...
  preload:function(){
    ...
    game.load.spritesheet('txt1', game.pathText+'txt1.png',194*game.mratio,104*game.mratio);
  ...
  create:function(){
    ...
    this.delayFlip = 2000;  // delai retournement quand cartes correpondent pas
  ...
  tweenAndText:function(f){
    ...
    tweentext.onComplete.add(function(){
      ...
      if (f == 1) this.cardmatch.play('', 0, .3, false, true);
      if (f == 2) {
        this.music.stop();
        this.allcards.play('', 0, .3, false, true);
      }
      var endtweentext = game.add.tween(this.texte.scale).to( { x:.1, y:.1 }, 200, Phaser.Easing.Elastic.In, true, this.delayFlip/1.8);
