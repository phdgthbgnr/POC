var bootState = {
    ...
    create:function(){
        ...
        game.get2Scale = function(){
          var ratio = 0;
          if (game.wdpr != 1) ratio = game.wdpr;
          if (game.wdpr == 1 && !game.ismobile) ratio = game.pratio;
          if (game.wdpr == 1 && game.ismobile) ratio = game.pratio/game.scalewh;
          return ratio;
        }

        game.set2ScaleP = function(sprt,s){
          s = s > 0 && s < 1 ? s : 1;
          if (game.wdpr != 1) sprt.scale.setTo(game.wdpr*s,game.wdpr*s);
          if (game.wdpr == 1 && !game.ismobile) sprt.scale.setTo(game.pratio*s,game.pratio*s);
          if (game.wdpr == 1 && game.ismobile){
            sprt.scale.setTo((game.pratio/game.scalewh)*s,(game.pratio/game.scalewh)*s);
          }
        }
        ...
        game.set2ButtonScaleP = function(sprt,s){
          if (game.wdpr != 1) sprt.scale.setTo(game.wdpr*game.wdpr*s,game.wdpr*game.wdpr*s);
          if (game.wdpr == 1) sprt.scale.setTo(game.pratio*s,game.pratio*s);
        }
        ...
        */
        ...
        game.addPauseButton = function(){
          ...
          game.set2Position(640,10,pause);
          pause.id='btpause';
        ...
        game.createDialogue = function(){
          ...
          game.dialogue1 = [[0,1,2],[3,4,5],[6,7,8]];     // transform goku - 3 dialogues par perso
        ...
        game.shuffleArray = function(a){
          for(var b=0;b<2;b++){ // shuffle 2 fois
            var j, x, i;
            for (i = a.length; i; i--) {
