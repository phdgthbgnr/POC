!function(i,t,m){
        ...
        _m.listenerAnimAdd(_currentTargetId,'animationend', function(e,t){
            ...
            scrollContentCenter(0, bsY, sY, sby, function(){
                ...
                if(_base.datas[idbase]['images'].length > 0) {
                    ...
                    let cw = _currentParent.querySelector('.carousel').getBoundingClientRect().width;
                    let uw = 0;
                    for(let i=0; i < _currentParent.querySelector('ul').children.length; i++){
                        uw += _currentParent.querySelector('ul').children[i].getBoundingClientRect().width;
                    }
                    console.log(cw + ' / ' + uw);
                    if(uw > cw){
                        _m.listenerAdd('carousel','mousewheel', function(e,c,t){
    ...
    const setWheelMouse = function(){
        if(!_isTouch){
            ...
            _m.listenerAdd('allthumb','mousewheel', function(e,c,t){
                if(_timeoutsrcoll) clearTimeout(_timeoutsrcoll);
                if(e.deltaY > 0){
                ...
                _timeoutsrcoll = setTimeout(function(){
